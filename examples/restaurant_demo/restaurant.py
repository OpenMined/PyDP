...
class RestaurantStatistics:
    ...
    def get_non_private_counts_per_hour(self) -> dict:
        ...
        valid_visits["Time entered"] = valid_visits["Time entered"].apply(lambda x: int(str(x)[:2]) if len(str(x)) == 4 else int(str(x)[:1]))
    ...
    def get_private_counts_per_hour(self, privacy_budget: float) -> dict:
        ...
        # Only count the hours without the minutes (12:00 and 12:30 both add 1 to the 12 count)
        visits = self._hour_visits.copy()
        visits["Time entered"] = pd.to_datetime(visits["Time entered"]).apply(lambda x: x.strftime(r'%H')).astype(int)
    ...
    def get_non_private_counts_per_day(self) -> dict:
        ...
        for day in self._day_visits["Day"].unique():
            day_counts[day] = self._day_visits[self._day_visits["Day"] == day]["Day"].count()
    ...
    def get_private_counts_per_day(self, privacy_budget: float) -> dict:
        ...
        for day in day_visits["Day"].unique():
            day_counts[day] = x.result(list(day_visits[day_visits["Day"] == day]["Day"]))
    ...
    def get_non_private_sum_revenue(self) -> dict:
        ...
        for day in self._day_visits["Day"].unique():
            day_revenue[day] = self._day_visits[self._day_visits["Day"] == day]["Money spent (euros)"].sum()
    ...
    def get_private_sum_revenue(self, privacy_budget: float) -> dict:
        ...
        for day in day_visits["Day"].unique():
            day_revenue[day] = int(x.result(list(day_visits[day_visits["Day"] == day]["Money spent (euros)"])))
    ...
    def get_private_sum_revenue_with_preaggregation(self, privacy_budget: float) -> dict:
        ...
        for day in day_visits["Day"].unique():
            ...
            visits_on_day = day_visits[day_visits["Day"] == day][["VisitorId", "Money spent (euros)"]]
...
def bound_visits_per_week(df, limit):
    ...
    id_to_visited = defaultdict(int)  # {visitor id: # of times it has visited the restaurant}
...
if __name__ == "__main__":
    r = RestaurantStatistics(hours_filename="day_data.csv", days_filename="week_data.csv")
