    ...
    def __init__(
        ...
        if not isinstance(sensitivity, numbers.Number):
            if not callable(sensitivity):
                raise TypeError(
                    f"Sensitivity  must be a number or callable. Got type {type(sensitivity)}."
                )
        ...
        if scale is not None:
            if not isinstance(scale, numbers.Number):
                raise TypeError(
                    f"Sensitivity  must be a int or float. Got type {type(sensitivity)}."
                )

        self.scale = scale

        if (cat_feat_idxs is not None and cat_sensitivity is None) or (
            cat_feat_idxs is None and cat_sensitivity is not None
        ):
            raise ValueError(
                "cat_feat_idxs cannot be None if cat_sensitivity, and vice versa."
            )

        self.categorical_exists = (
            cat_feat_idxs is not None and cat_sensitivity is not None
        )
        ...
        if self.categorical_exists:
            ...
            if len(cat_feat_idxs) == 0:
                raise ValueError(
                    "At least 1 categorical feature index must be provided."
                )
        ...
        for data_idx in range(n_data):
            for feature_idx in range(n_feature):
                ...
                feature = np.concatenate(
                    (X[:data_idx, feature_idx], X[data_idx + 1 :, feature_idx])
                )
                ...
                laplace = LaplaceDistribution(
                    epsilon=float(self.epsilon), sensitivity=float(sensitivity_)
                )
